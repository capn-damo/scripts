...
get_screen_dimensions() {   # get net workarea, if panels are present
    ...
    desktopH=$(xrandr -q | awk '/Screen/ {gsub(",","");print $10}')  # total desktop height
    ...
    WINPOS_X=$(xwininfo -id $WINDOW | grep "Absolute upper-left X")
    
    if [[ ${WINPOS_X##*' '} -gt $screenW1 ]];then # window is on R monitor
    ...
    # Y position of active window
    WINPOS_Y=$(xwininfo -id $WINDOW | grep "Absolute upper-left Y")
...
snap_top(){
    ...
    WIN_WIDTH_R=$((( $screenW / 2 ) - $MARGIN_R - $adjust_X ))
    WIN_WIDTH=$(( $screenW ))
    WIN_HEIGHT=$(( ( $valH / 2 ) - $adjust_Y ))
    #WIN_HEIGHT=$(( $valH / 2 ))
    
    wmctrl -r :ACTIVE: -b remove,maximized_vert && \
    wmctrl -r :ACTIVE: -b add,maximized_horz && \
    wmctrl -r :ACTIVE: -e 0,-1,"$YPOS",-1,"$WIN_HEIGHT"
}
...
snap_bottom(){
    ...
    wmctrl -r :ACTIVE: -b remove,maximized_vert && \
    wmctrl -r :ACTIVE: -b add,maximized_horz && \
    wmctrl -r :ACTIVE: -e 0,"$XPOS","$YPOS",-1,"$WIN_HEIGHT"
}
####    END FUNCTIONS   ################################################
